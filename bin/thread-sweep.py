...
def buildParser():
    ...
    parser.add_argument('--cores', '-C', type=parseIntCommaList, required=True,
                        help="A comma separated list of #cores to test")
    parser.add_argument('--oversub', '-S', type=parseIntCommaList, required=True,
                        help="A comma separated list of thread oversubscription ratio to test")
    parser.add_argument('--cherrypick', type=parseCherryPickedConf,
...
def sweep(args, csvWriter, rowCallback: Callable[[], None]):
    ...
    allConfs = itertools.product(args.cores, args.oversub) + args.cherrypick
    for p in packages:
        for (ncores, oversub) in allConfs:
            for trialID in range(args.ntrials):
                ...
                cmd = [
                    ...
                    "-d", args.rundir, "-s", timer_cmd, "-k", "--unpack"
                ]
